  ...
  run: async ({ title, youtubeId, thumbnail, accessToken, channel }) => {
    ...
    try {
      ...
      const message = await authClient.chat.postMessage(
        channel,
        `:tv:\nDirect YouTube link to TheFamily's newest video: <http://startupfood.meteor.com/video/${youtubeId}|https://www.youtube.com/watch?v=${youtubeId}>`,
        {
          username: Meteor.settings.public.botName,
          as_user: false,
          ...
          attachments: [
            {
              fallback: `New TheFamily video online - ${title}`,
              title: `${title}`,
