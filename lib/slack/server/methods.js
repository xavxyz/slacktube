...
const requestSlackToken = new ValidatedMethod({
  ...
  // by now, call it with Meteor.call & use promisify-core package from maxime
...
const distributeVideo = new ValidatedMethod({
  ...
  validate: null,
  run: async ({ video, accessToken, channel }) => {
    ...
    try {
      ...
      const message = await authClient.chat.postMessage(
        channel,
        `:tv:\nDirect YouTube link to TheFamily's newest video: <http://startupfood.meteor.com/video/${video.youtubeId}|https://www.youtube.com/watch?v=${video.youtubeId}>`,
        {
          username: Meteor.settings.public.botName,
          as_user: false,
